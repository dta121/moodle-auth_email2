...
/**
 * Authentication Plugin: Email2 Authentication
 *
...
/**
 * Email2 authentication plugin.
 */
class auth_plugin_email2 extends auth_plugin_base {

    /**
     * Constructor.
     */
    public function __construct() {
        $this->authtype = 'email2';
        $this->config = get_config('auth_email2');
    }
     ...
     */
    public function auth_plugin_email2() {
    ...
    public function user_signup_with_confirmation($user, $notify=true, $confirmationurl = null) {
        ...
        if (!empty($SESSION->wantsurl)) {
            set_user_preference('auth_email2_wantsurl', $SESSION->wantsurl, $user);
        }
        ...
        if (! send_confirmation_email($user, $confirmationurl)) {
            print_error('auth_emailnoemail', 'auth_email2');
        }
    ...
    function user_confirm($username, $confirmsecret) {
        ...
        if (!empty($user)) {
            if ($user->auth != $this->authtype) {
                ...
                if ($wantsurl = get_user_preferences('auth_email2_wantsurl', false, $user)) {
                    // Ensure user gets returned to page they were trying to access before signing up.
                    $SESSION->wantsurl = $wantsurl;
                    unset_user_preference('auth_email2_wantsurl', $user);
                }
